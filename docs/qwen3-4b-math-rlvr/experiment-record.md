...
## 3. 评测结果
### 3.1 Benchmark 完整对比
| Step | aime_2024 | aime_2025 | gpqa_diamond | gsm8k | livecodebench_v5 | math_500 | Avg |
|---|---|---|---|---|---|---|---|
| base | 77.08 | 70.00 | 55.43 | 95.22 | 54.37 | 94.20 | 74.38 |
| 99 | 79.17 | 69.58 | 55.81 | 94.92 | 53.09 | 94.50 | 74.51 (+0.13) |
| 199 (best avg) | 79.17 | 73.33 | 57.13 | 95.00 | 55.65 | 94.45 | 75.79 (+1.41) |
| 299 | 81.25 | 69.58 | 56.38 | 95.00 | 55.35 | 94.13 | 75.28 (+0.90) |
| 399 | 77.50 | 72.92 | 55.24 | 94.77 | 56.17 | 94.25 | 75.14 (+0.76) |
| 499 | 77.08 | 73.75 | 55.93 | 95.07 | 55.80 | 94.25 | 75.31 (+0.93) |
| 599 | 79.58 | 71.67 | 54.80 | 94.84 | 55.12 | 94.38 | 75.06 (+0.68) |
| 699 | 79.58 | 70.83 | 54.61 | 95.07 | 55.87 | 94.15 | 74.95 (+0.57) |
| 799 | 76.67 | 70.00 | 56.50 | 94.77 | 55.20 | 94.43 | 74.59 (+0.21) |
...
关键发现：
- aime_2024: step299 达到 81.25，与 POLARIS-4B 的 81.2 持平，但随后持续回落（599/699=79.58, 799=76.67）
- aime_2025: 最高 73.75（step499），之后回落至 70.00（step799，完全退回 base 水平）
- Avg 在 step199 达到峰值 75.79，之后单调下降 → step799 仅 74.59，接近 base
- step599 之后所有指标全面退化，确认 entropy collapse 导致过拟合不可逆
...
### 3.4 Deepinsight 完整分析（1868 步）
从训练日志提取全量 deepinsight_algorithm 指标，覆盖 1868 步完整训练过程。

| 指标 | 初始值 | 末期值 | 说明 |
|---|---|---|---|
| kl_loss | -0.0004 | -0.00001 | 全程 ≈ 0，KL 约束完全失效 |
| entropy | 6.76 | 0.49 | 下降 93%，step 47 跌破 1.0 |
| reward | 0.346 | 0.751 | 训练集饱和 |
| clip_ratio | 0.0 | 0.0 | 全程为 0，PPO clipping 从未触发 |
| response_length | 3204 | 4466 | 模型学会写更长但重复的模板 |
| grad_norm | 0.018 | 0.012 | 梯度逐渐消失 |
| policy_loss | ~-0.0001 | ~-0.00001 | 趋近于 0 |

关键发现：
- entropy 在 step 47 就跌破 1.0 — 训练最初几十步就丧失探索能力
- clip_ratio 全程 = 0 — importance weight 始终在 [0.8, 1.2] 内，PPO clip 机制完全没生效
- kl_loss 和 entropy 完全脱钩 — entropy 暴跌但 KL 不变，说明 kl_ctl=0.001 无法约束策略漂移

![Deepinsight 完整分析](charts/deepinsight_full_analysis.png)

### 3.5 Zero-Advantage 分析
每步 batch = 16 prompts × 8 samples = 128 sequences。当一组 8 个 sample 全对（8/8）或全错（0/8）时，group-relative advantage = 0，该组不产生梯度。

| 训练阶段 | Correct% | 8/8 全对 (no grad) | 0/8 全错 (no grad) | zero-advantage 合计 | 有效梯度 |
|---|---|---|---|---|---|
| step 1-50 | 36.9% | 18% | 20% | 38% | 62% |
| step 50-200 | 58.8% | 36% | 12% | 49% | 51% |
| step 200-500 | 71.5% | 47% | 8% | 55% | 45% |
| step 500+ | 74.4% | 49% | 7% | 56% | 44% |

训练后期超过一半的 batch 不产生任何梯度信号。这解释了为什么 step200 后 eval 改善停滞 — 有效学习信号越来越少。

---

## 4. 实验分析
### 4.1 核心结论
1. 训练效果一般 — Avg 最高仅 +1.41%（step199），AIME24/25 无法同时提升，step599 后全面退化
2. 训练集 reward 快速饱和（70%+）但评测集提升有限 — 典型的过拟合信号
3. entropy 从 ~7 暴跌至 ~0.5（下降 93%），step 47 就跌破 1.0 — 模型几乎立即丧失探索能力
4. KL 散度几乎不变（~0.0003），clip_ratio 全程 = 0 — kl_ctl=0.001 和 PPO clip 均未起作用
5. 训练后期 56% 的 batch 是 zero-advantage（8/8 全对 + 0/8 全错）— 有效梯度信号严重不足
