...
1. **Run 3 如果 reward 不升，考虑以下调整**（只改一个变量）：

   - **方案 A**: 混合数据——dapo_math (简单, reward~0.83) + deepmath (难, reward~0.19) 构造 0.3-0.5
     的理想区间
...
1. **尝试 DAPO 算法**——当前 GRPO 的 entropy 管控明显不足，DAPO 的 Clip-Higher + Dynamic Sampling 可能更合适
1. **尝试 POLARIS 风格配置**——高温 (1.4)、无 KL、数据过滤 (移除通过率>90% 的题)
