...
在 Qwen3-8B 上打通 Math SFT → RLVR → 轨迹回灌 完整后训练闭环，验证 RL 能持续提升数学推理能力。详见
...
注意：`prompt` 是 messages 列表格式（非纯文本），无 `solution` 列。当前 RLVR 配置使用 dapo_math_17k；如需切换到
deepmath_20k，需适配 RL dataset loader。
...
**Q1: 哪个模型做 RL 起点最好？** → **Instruct (M2)**。平均 76.24%，全面最高。AIME24 79.58% 比 POLARIS 的
baseline (73.8%) 还高。

**Q2: SFT 是否有效？** → 非常有效。Base → SFT: 34.64% → 71.79% (+37.15%)，AIME24 从 12% 到 71%（+59%）。

**Q3: Instruct vs SFT-4450？** → Instruct 全面领先（+4.45% avg），但 Math 500 上 SFT-4450
反超（94.98% vs 94.43%）。纯数学能力接近持平，Instruct 在推理泛化（GPQA）和代码（LiveCode）上更强。

**Q4: SFT 训练是否饱和？** → 接近饱和。3905→4450 在 AIME25 上 +11.25%（大幅提升），但 AIME24
-0.83%（微降）。继续训练预期收益递减。

**Q5: 格式合规率？** → 所有后训练模型 GSM8K > 94%，格式基本稳定。\[待补充\] `\boxed{}` 专项检查。

**Q6: 基础能力如何？** → 所有后训练模型 GSM8K > 94%，基础能力无问题。Base 的 81.50% 也在可接受范围。
...
1. POLARIS 用同类起点验证了 Instruct → 直接 RL 有效
1. 格式最稳（原生 thinking 模式 + `\boxed{}`），RL 崩坏风险最低
1. 失败成本低——切到 SFT-4450 只需改 `actor.path`

回退条件：

- RL 100 步后 reward 不升 + entropy collapse → 切 SFT-4450
- SFT-4450 路径:
  `/dataset_rc_b1/llm_train_sft/lijl42/SFT_Qwen3_8B_Base/qwen3_8b_base_ot3_sft_0105/global_step_4450/huggingface`
...
1. **开 thinking 后 Instruct reward 追平 SFT-4450**（0.594 vs 0.594），CoT 推理有效
1. **SFT-4450 自发 CoT**（response 6.5k），不需要额外开 thinking
1. ② 和 ③ 表现完全一致（reward、incorrect 数量都相同），说明两种起点 + CoT 等效
1. ① 速度最快但效果最差——Instruct 不开 thinking 做 RLVR 不可行
...
### Run 2 评测结果 (DeepInsight, 6 数据集, dapo_math_17k 训练)

**完整评测矩阵（所有模型 × 所有 step）：**

| 数据集 | ③ think P0 | ③ step99 | ③ step199 | ③ step499 | ① nothink P0 | ① step99 | ① step199 | ② sft4450 P0 | ② step99 | ② step199 |
|---|---|---|---|---|---|---|---|---|---|---|
| AIME 2024 | 79.58 | 80.83 | 77.92 | 72.92 | 79.58 | 78.75 | 77.92 | 71.25 | 72.92 | 70.00 |
| AIME 2025 | 70.83 | 70.00 | 70.83 | 63.33 | 70.83 | 72.92 | 69.17 | 65.00 | 58.75 | 61.25 |
| GPQA | 61.55 | 61.36 | 62.31 | 59.34 | 61.55 | 61.81 | 61.17 | 56.57 | 56.00 | 55.49 |
| GSM8K | 95.38 | 95.91 | 96.29 | 95.83 | 95.38 | 95.30 | 95.75 | 94.77 | 95.15 | 95.00 |
| LiveCode | 55.65 | 56.33 | 56.63 | 53.69 | 55.65 | 57.15 | 57.30 | 48.19 | 47.21 | 44.50 |
| Math 500 | 94.43 | 94.15 | 94.45 | 93.88 | 94.43 | 94.68 | 92.98 | 94.98 | 95.20 | 94.30 |
| **平均** | **76.24** | **76.43** | **76.40** | **73.16** | **76.24** | **76.77** | **75.72** | **71.79** | **70.87** | **70.09** |

**各模型最佳 checkpoint 汇总：**

| 模型 | 最佳 step | 平均 Acc | vs P0 基线 | 核心指标变化 |
|---|---|---|---|---|
| ③ Instruct+think | step 99 | 76.43% | +0.19% | AIME24 +1.25%, LiveCode +0.68% |
| ① Instruct nothink | step 99 | 76.77% | +0.53% | AIME25 +2.09%, LiveCode +1.50% |
| ② SFT-4450 | step 99 | 70.87% | -0.92% | Math500 +0.22%, 其余下降 |

**① Instruct nothink step 500 评测失败**——模型 checkpoint 已崩溃（entropy 爆炸后参数损坏）。

**② SFT-4450 step 500 评测**——待补充（已提交，结果未出）。

### Run 2 结论

1. **RL 在 dapo_math_17k 上几乎无正向效果**：全部三组实验的最佳 checkpoint 均在 step 99，提升极微（+0.19% ~ +0.53% avg）
2. **意外发现：nothink step 99 (76.77%) 略优于 think step 99 (76.43%)**——nothink 在 AIME25 +2.09%、LiveCode +1.50%，但这可能是评测噪声（差距太小）
3. **step 200 后全部持续退化**：③ think step 499 平均 -3.08%
4. **三个实验全部在 step ~500-600 entropy 爆炸后崩溃**（① nothink step 500 checkpoint 无法加载）
1. **三个实验全部在 step ~500-600 entropy 爆炸后崩溃**
1. **根本原因：训练集太简单**——Instruct 在 dapo_math_17k 上 step 1 task_reward=0.83（128 样本中 106
   个做对），GRPO group 内 advantage 几乎无区分度
1. **最佳模型是 ③ Instruct+think step 99**（76.43% avg, AIME24 80.83%），但提升微弱
1. **SFT-4450 全面退化**，RL 没有帮助

### Run 3: deepmath_20k 实验 (2026-04-13)

基于 Run 2 结论，切换到更难的 deepmath_20k 数据集。共提交三个实验：

| 实验 | Job | 模型 | reward 修复 | 步数 | 状态 |
|---|---|---|---|---|---|
| v1 Instruct+think | `bifrost-2026041311175401-zengbw1` | Instruct+think | 未修复(旧) | 839 | RUNNING |
| v2 Instruct+think | `bifrost-2026041315262101-zengbw1` | Instruct+think | 已修复 | 678 | RUNNING |
| SFT-4450 | `bifrost-2026041315040001-zengbw1` | SFT-4450 | 已修复 | 498 | RUNNING |

**Reward bug 修复（2026-04-13）：** deepmath 的 answer 有 80% 是非数值（Yes/No/True/False/符号表达式），math_verify 无法解析导致 reward 永远返回 0。修复：answer 包裹 `\boxed{}`，reward 函数添加字符串匹配 fallback。v2 使用修复后的代码。

**训练曲线对比（每 50 步采样）：**

v2 Instruct+think（修复后）：
| step | task_reward | entropy | incorrect/128 | seq_len |
|---|---|---|---|---|
| 1 | 0.7266 | 0.3773 | 35 | 6822 |
| 50 | 0.5625 | 0.3678 | 56 | 8296 |
| 100 | 0.5469 | 0.3967 | 58 | 7872 |
| 200 | 0.6719 | 0.3853 | 42 | 7540 |
| 300 | 0.7734 | 0.4581 | 29 | 6279 |
| 400 | 0.8047 | 0.7154 | 25 | 5896 |
| 450 | 0.4922 | 4.2670 | 65 | 3995 |
| 500 | 0.6328 | 1.7599 | 47 | 3816 |
| 600 | 0.7031 | 0.1275 | 38 | 5691 |
| 678 | 0.7109 | 0.1847 | 37 | 5305 |

SFT-4450（修复后）：
| step | task_reward | entropy | incorrect/128 |
|---|---|---|---|
| 1 | 0.1641 | 0.7323 | 107 |
| 100 | 0.0078 | 0.7269 | 127 |
| 200 | 0.1094 | 0.7745 | 114 |
| 300 | 0.1641 | 0.7807 | 107 |
| 450 | 0.1016 | 2.3298 | 115 |
| 498 | 0.1484 | 1.0681 | 109 |

v1 Instruct+think（未修复）：
| step | task_reward | entropy | incorrect/128 |
|---|---|---|---|
| 1 | 0.1875 | 0.3526 | 104 |
| 200 | 0.1719 | 0.3297 | 106 |
| 400 | 0.2109 | 0.3713 | 101 |
| 600 | 0.0391 | 0.2713 | 123 |
| 839 | 0.2891 | 0.5154 | 91 |

### Run 3 结论

| 实验 | 初始 task_rw | 峰值 | 最终 | 趋势 | 关键发现 |
|---|---|---|---|---|---|
| v2 Instruct+think | 0.73 | 0.98 (step 218) | 0.71 | 平稳 | reward 修复有效；entropy 稳定但 step 450 有 spike |
| SFT-4450 | 0.16 | 0.50 | 0.15 | 平稳 | deepmath 对 SFT-4450 太难 |
| v1 (旧代码) | 0.19 | 0.55 | 0.29 | 微升 | reward bug 导致指标虚低，不可用 |

1. **Reward 修复验证成功**：v1 task_reward=0.19 vs v2=0.73，同模型同数据，差异完全来自 bug 修复
2. **v2 Instruct+think 表现最好但仍无持续提升**：初始 0.73，峰值 0.98，最终回到 0.71。RL 没有带来超越初始模型的改进
3. **SFT-4450 + deepmath 确认不可行**：task_reward=0.16，128 样本中只有 21 个做对
4. **entropy 稳定性大幅改善**：相比 Run 2 dapo_math（step 500 爆炸到 3-10），deepmath 上 entropy 整体可控（v2 除 step 450 spike 外稳定在 0.1-0.7）
5. **核心问题不变**：Instruct 在 deepmath 上正确率 73%（偏高），RL 提升空间有限
| entropy (step 1-66) | 0.28→爆炸           | 0.31-0.36 (稳定)    | 没有 entropy 爆炸 |
| reward 趋势         | 先降后崩            | 震荡，未见明显上升  | 尚无正向信号      |
| incorrect/128       | 18-30               | 94-117              | 大部分题做错      |

**观察：** deepmath 难度合适但偏高，entropy 稳定是好消息。需要继续观察到 200+ 步看是否出现上升趋势。如果 reward 持续不升，可能需要：混合
dapo_math (简单) + deepmath (难) 构造理想难度分布，或提高 n_samples (8→16)。
...
| 2026-04-14 | Run 2+3 全部评测完成，RL 训练未产生实质提升 | 6 组实验、2 数据集、3 模型设定，最佳 Δ 仅 +0.53%（噪声范围）。根因：数据难度不匹配（dapo 83% 太简单，deepmath 73% 偏高） | 确认 |
| 2026-04-14 | 停掉 Run 3 全部训练任务 | v2 think 678 步无持续提升；sft4450 498 步 reward=0.15 太难；v1 旧代码 bug 不可用 | 确认 |
| 2026-04-14 | 下一步：数据难度校准（方案 1，最高优先级） | 参考 POLARIS：用模型 rollout 评估每题通过率，保留 10-60% 区间的题，构造精选数据集 | 待执行 |
...
经过 Run 1-3 共 6 组实验、2 个数据集、3 种模型设定、跨 5 天的训练和评测，核心结论是：

> **RL pipeline 功能完整，但训练没有产生实质性的 benchmark 提升。所有实验最佳 checkpoint 与 P0 基线的差异都在 ±1% 以内（评测噪声范围）。**

| Run | 数据集 | 实验数 | 最佳 Δ vs 基线 | 问题 |
|---|---|---|---|---|
| Run 2 | dapo_math_17k | 3 | +0.53% (nothink s99) | 数据太简单(83%)，entropy 爆炸 |
| Run 3 | deepmath_20k | 3 | +0% (v2 think) | 修复后正确率 73% 仍偏高，无持续提升 |

### 8.2 问题逐层分析

**问题 1：数据难度不匹配（根本原因）**

| 数据集 | Instruct 正确率 | SFT-4450 正确率 | 理想区间 | 差距 |
|---|---|---|---|---|
| dapo_math_17k | 83% | 77% | 20-50% | 太简单 |
| deepmath_20k | 73% | 16% | 20-50% | Instruct 偏高，SFT 太难 |

POLARIS 论文明确指出：训练数据中模型正确率需在 20-50%，并且要过滤掉通过率 >90% 和 =0% 的题。我们两个数据集都不在这个区间。

GRPO 的 group advantage 计算依赖同一 prompt 的 n_samples 个回复有正有错。当正确率 >70% 时，8 个回复中 5-6 个正确，advantage 的方差极小，梯度信号弱且噪声大。

**问题 2：序列长度限制（与 v070 的关键差异）**

| 参数 | AReaL 当前 | v070 | 影响 |
|---|---|---|---|
| max_response_length | 8192 | 31744 | 限制了长链推理，模型无法充分展开思考 |
| context_length | 16384 | 32768 | 同上 |

POLARIS 用 52K 训练、90K 推理。我们只有 8K，对于需要长思考链的难题，模型被截断后答案不完整，导致 reward 被错误地设为 0（no_eos_ratio 在 Run 2 中达到 12.5%）。

**问题 3：超参数偏保守**

| 参数 | AReaL 当前 | POLARIS | 影响 |
|---|---|---|---|
| temperature | 0.99 | 1.4-1.5 | 采样多样性不足，pass@k 上限低 |
| gradient_clipping | 1.0 | 未公开（但用了 Clip-Higher） | 大梯度未被裁剪 |
| n_samples | 8 | 8 | 一致，但当正确率 >70% 时 8 个不够 |
| kl_ctl | 0.001 | 0.0 | KL 约束可能限制了探索 |

**问题 4：缺少 DAPO 的关键保护机制**

| DAPO 技术 | 作用 | 当前状态 |
|---|---|---|
| Clip-Higher (eps_clip_high=0.28) | 允许更大幅度远离差策略 | 未使用（对称 clip=0.2） |
| Dynamic Sampling | 过滤 reward 全相同的 group | 未使用 |
| Overlong Mask | 截断序列排除出 loss | 未使用 |
| 无 KL + 无 entropy loss | 自由探索 | 有 KL=0.001 |

Run 2 的 entropy 爆炸可能部分归因于缺少这些机制——DAPO 通过 Clip-Higher 替代 KL 来约束策略更新幅度，同时允许更大的探索空间。

### 8.3 与业界成功案例的差距分析

**POLARIS (Qwen3-4B → AIME24 81.2%) 做对了什么：**

| POLARIS 做法 | 我们的现状 | 差距 |
|---|---|---|
| 数据过滤：移除通过率 >90% 的题 | 未过滤 | 核心差距 |
| 模型级数据校准：每题 8 次 rollout 评估难度 | 未做 | 核心差距 |
| 高温采样 (1.4-1.5) | 0.99 | 多样性不足 |
| Train-Short Generate-Long (52K→90K) | 8K | 长序列能力受限 |
| 多阶段训练（温度递增 + 数据过滤迭代） | 单阶段 | 训练策略过于简单 |
| Intra-Batch Substitution（替换无信息 batch） | 无 | 训练效率低 |
| 30K 精选数据 (从 53K 过滤) | 17K/20K 全量 | 数据质量差异 |

**Skywork-OR1 (Qwen2.5-32B → +15% avg) 做对了什么：**

- MAGIC entropy 调度：多阶段自适应 entropy 控制
- 开源 105K 数学 + 14K 代码数据集，经过质量筛选
- 从 base+SFT 起步而非 instruct（更高 entropy 起点）

**DeepSeek-R1 做对了什么：**

- 四阶段流水线：R1-Zero → SFT → RLVR → RLHF
- R1-Zero 从 base model 纯 RL，保留最大探索空间
- 回灌高质量轨迹做 SFT，再 RL

### 8.4 建议方案（按优先级排序）

**方案 1（最推荐）：数据难度校准**
- 用 Instruct+think 在 deepmath_20k 上 rollout 8 次
- 统计每题通过率，保留 10-60% 的题
- 预期得到 5-10K 精选数据，每题都有有效 reward 信号
- 工程量：写 1 个脚本（离线 rollout + 过滤），1-2 天
- 风险：最低，POLARIS 已验证

**方案 2：拉长序列 + 提高温度**
- max_new_tokens: 8192 → 16384 或 32768
- temperature: 0.99 → 1.2
- 用 v070 bench 配置（`qwen3_8b_rlvr_bench_v070.yaml`）直接跑
- 工程量：改 YAML，0.5 天
- 风险：中，可能 OOM 或训练速度大幅下降

**方案 3：切 DAPO 算法**
- eps_clip_higher=0.28, kl_ctl=0.0, mask_no_eos_with_zero=true, dynamic_bs=true
- 工程量：改 YAML 几行，0.5 天
- 风险：中，DAPO 需要配合数据过滤效果才好

**方案 4：从 Base+SFT 起步**
- 用 OpenThoughts3 SFT 训练的 checkpoint 做 RL（如果 SFT 任务已完成）
- Base+SFT 没有 RLHF 的 entropy 压缩，探索空间更大
- 工程量：等 SFT 完成 + 改 actor.path
- 风险：中，SFT 质量不确定

**建议执行顺序**：方案 1 → 方案 2+3 组合 → 方案 4

方案 1 解决根本问题（数据难度），方案 2+3 优化训练效率，方案 4 是更大的变更需要更多验证。先解决数据问题，其他优化才有意义。
...
- Docker:
  `infra-registry-vpc.cn-wulanchabu.cr.aliyuncs.com/data-infra/fuyao:zhangjh37-260325-0644`
...
- Config: `fuyao_examples/math/qwen3_4b_rlvr.yaml` / `code_dapo/code_dapo_qwen3_4b.yaml`
  / `search_r1/search_r1_qwen3_4b.yaml`
