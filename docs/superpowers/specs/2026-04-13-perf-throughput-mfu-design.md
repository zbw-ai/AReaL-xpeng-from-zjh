...
注：`actor.py` 不需要修改。token 数从 `stats_tracker.export_all()` 返回的 `ppo_actor/n_tokens__count` 中获取（stats_tracker 在导出分母时自动生成 `__count` 后缀字段）。
...
不修改 `actor.py`。Token 数从已有的 stats_tracker 机制中获取：

`actor.py` 的 `_ppo_update()` 在第 271 行已经通过 `stats_tracker.denominator(n_tokens=...)` 注册了 token 数。`stats_tracker.export_all()` 导出时自动生成 `ppo_actor/n_tokens__count` 字段（int 类型），即为该步的 token 总数。

在 `rl_trainer.py` 的步结束阶段：
```python
stats = self.actor.export_stats()  # 已有代码
train_tokens = stats.get("ppo_actor/n_tokens__count", 0)
```

这样零侵入 actor.py，复用已有数据流。
...
| `ppo_actor/n_tokens__count` 可能不存在 | token 数获取失败 | 加 `.get()` 默认值保护；首次运行时确认字段存在 |
